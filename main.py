...
def get_available_categories_from(website):
    return {
        '1377x': tssx.navigation.tssx_get_available_categories()
    }[website]
...
def get_json_data_from(website, search_term, category):
    if website == '1377x':
        return tssx.parser.tssx_page_to_json(tssx.navigation.tssx_search(search_term, category))
...
def download(website, relative_torrent_link):
    if website == '1377x':
        subprocess.run(
            ['qbittorrent', tssx.navigation.tssx_get_magnet_link(relative_torrent_link)])
